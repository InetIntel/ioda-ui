...
const ControlPanel = ({
  ...
  const getCountryNamesFromAsn = useCallback(
    async (geoasnCode) => {
      try {
  ...
  const getCountryInfoFromGeoAsn = useCallback(
    async (geoasnCode) => {
      try {
  ...
  const getAsnNetworkEntity = useCallback(
    async (asnCode) => {
      try {
        let url;
        if (asnCode.includes("-")) {
        ...
        if (entityType === "asn" && entityCode) {
          ...
          if (countryNames && countryNames.length > 0) {
            results = results.filter((item) =>
              countryNames.includes(item.label)
            );
          ...
          if (countryCode !== "") {
            results = results.filter(
              (item) => countryCode === item.entity.code
            );
  ...
  useEffect(() => {
    ...
    const fetchAsnNetworks = async () => {
      try {
        ...
        let selectedNetwork;
        if (results.length > 0) {
          selectedNetwork = await results.find(
            (asn) => asn.entity.code == entityCode
          );
        }
    ...
    if (entityType === "region" && entityCode && regionOptions.length > 0) {
      const fetchRegionData = async () => {
        ...
        try {
          ...
          const region = await regionOptions.find(
            (region) => region.entity.code == entityCode
          );
  ...
  useEffect(() => {
    if (entityType === "asn" && entityCode && asnOptions.length > 0) {
        ...
        try {
          if (entityCode.includes("-")) {
            ...
            if (isNaN(geoCode)) {
              ...
              // region
              const selectedRegion = await regionOptions.find(
                (region) => region.entity.code == geoCode
              );
            ...
            const asn = await asnOptions.find(
              (asn) => entityCode === asn.entity.code
            );
  ...
  function handlePredefinedRangeSelection({ value }) {
    if (value === RANGES.LAST_60_MINS) {
      handleRangeChange([getNowAsUTC().subtract(60, "minute"), getNowAsUTC()]);
    } else if (value === RANGES.LAST_24_HOURS) {
      handleRangeChange([getNowAsUTC().subtract(24, "hour"), getNowAsUTC()]);
      ...
      handleRangeChange([getNowAsUTC().subtract(30, "day"), getNowAsUTC()]);
  ...
  function handleCountrySelectChange(id) {
    if (id === undefined) {
      if (countrySearchText !== "N/A") {
        const entity =
          countryOptions.find((d) => d.name === countrySearchText)?.entity ??
          null;
        if (entity != null) {
  ...
  function handleRegionSelectChange(id) {
    if (id === undefined) {
      if (regionSelectedCode) {
        const entity =
          regionOptions.find((d) => d.entity.code === regionSelectedCode)
            ?.entity ?? null;
        if (entity != null) {
          handleEntityChange(entity.backUrl);
        }
  ...
  function handleAsnSelectChange(id) {
    if (id === undefined) {
      if (asnSelectedCode) {
        const entity =
          asnOptions.find((d) => d.entity.code == asnSelectedCode)?.entity ??
          null;
        if (entity != null) {
          handleEntityChange(entity.backUrl);
        }
  ...
  const debouncedHandleAsnSearch = useCallback(debounce(handleAsnSearch, 200), [
    asnOptions,
  ]);
  ...
  const fetchAsnMatchingOptions = async (asnSearchText) => {
    ...
    try {
      ...
      const fetched = await fetchData({ url });
      const resultsArray = Array.isArray(fetched?.data?.data)
        ? fetched.data.data
        : [];
      const results = await Promise.all(
  ...
  const fetchRegionMatchingOptions = async (regionSearchText) => {
    ...
    try {
      ...
      console.error(
        "Failed to fetch region options based on search text:",
        error
      );
    } finally {
      ...
      >
        ...
        <div className="control-panel__controls">
          ...
          <Select
            ...
            onSearch={(value) => {
              if (
                countrySelectedCode === "all_countries" &&
                asnSelectedCode === "all_networks"
              ) {
